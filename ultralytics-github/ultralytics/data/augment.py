...
from ultralytics.data.utils import polygons2masks, polygons2masks_overlap, polygons2masksparsing, polygons2masksparsing_overlap
from ultralytics.utils import LOGGER, colorstr
from ultralytics.utils.checks import check_version
from ultralytics.utils.instance import Instances, InstancesParsing, InstancesParsingWBC
...
class MosaicParsing(BaseMixTransform):
    """
    Mosaic augmentation.
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        final_labels = {
            "im_file": mosaic_labels[0]["im_file"],
            "ori_shape": mosaic_labels[0]["ori_shape"],
            "num_part": mosaic_labels[0]["num_part"],
            "resized_shape": (imgsz, imgsz),
            "cls": np.concatenate(cls, 0),
            "instances": InstancesParsing.concatenate(instances, axis=0),
            "mosaic_border": self.border,
        }
        final_labels["instances"].clip(imgsz, imgsz)
        good = final_labels["instances"].remove_zero_area_boxes()
...
class MosaicParsingWBC(BaseMixTransform):
    """
    Mosaic augmentation.
    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            ...
            # Load image
            img = labels_patch["img"]
            segments = labels_patch['instances'].segments
            h, w = labels_patch.pop("resized_shape")

            # Place img in img4
            if i == 0:  # top left
                img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                segments4 = np.full((segments.shape[0], segments.shape[1], s * 2, s * 2), 0, dtype=np.uint8)
                x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc  # xmin, ymin, xmax, ymax (large image)
                x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h  # xmin, ymin, xmax, ymax (small image)
            elif i == 1:  # top right
                x1a, y1a, x2a, y2a = xc, max(yc - h, 0), min(xc + w, s * 2), yc
            ...
            pdb.set_trace()
            img4[y1a:y2a, x1a:x2a] = img[y1b:y2b, x1b:x2b]  # img4[ymin:ymax, xmin:xmax]
            segments4[:,:,y1a:y2a, x1a:x2a] = segments[:,:,y1b:y2b, x1b:x2b]
            padw = x1a - x1b
            padh = y1a - y1b
    ...
    def _update_labels(labels, padw, padh):
        ...
        labels["instances"].convert_bbox(format="xyxy")
        labels["instances"].denormalize(nw, nh)
        labels["instances"].add_padding_instance(padw, padh)
        return labels
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        final_labels = {
            ...
            "resized_shape": (imgsz, imgsz),
            "cls": np.concatenate(cls, 0),
            "instances": InstancesParsingWBC.concatenate(instances, axis=0),
            "mosaic_border": self.border,
        }
        final_labels["instances"].clip(imgsz, imgsz)
        good = final_labels["instances"].remove_zero_area_boxes()
...
class MosaicParsingGroup(MosaicParsing):
    def __init__(self, dataset, imgsz=640, p=1.0, n=4):
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        for labels in mosaic_labels:
            cls.append(labels["cls"])
            group.append(labels["group"] if not group else labels["group"]+group[-1].max()+1)
            instances.append(labels["instances"])
            pathological_cls.append(labels["pathological_cls"])
        # Final labels
        #pdb.set_trace()
        final_labels = {
            "im_file": mosaic_labels[0]["im_file"],
            ...
            "resized_shape": (imgsz, imgsz),
            "cls": np.concatenate(cls, 0),
            "group": np.concatenate(group, 0),
            "instances": InstancesParsing.concatenate(instances, axis=0),
            "pathological_cls": np.concatenate(pathological_cls, 0),
            "mosaic_border": self.border,
        }
        final_labels["instances"].clip(imgsz, imgsz)
        good = final_labels["instances"].remove_zero_area_boxes()
        final_labels["cls"] = final_labels["cls"][good]
        final_labels["group"] = final_labels["group"][good]
        final_labels["pathological_cls"] = final_labels["pathological_cls"][good]
        if "texts" in mosaic_labels[0]:
            final_labels["texts"] = mosaic_labels[0]["texts"]
        return final_labels
    
class MosaicParsingGroupMatch(MosaicParsing):
    def __init__(self, dataset, imgsz=640, p=1.0, n=4):
        super().__init__(dataset=dataset, imgsz=imgsz, p=p, n=n)
...
class MixUpParsing(BaseMixTransform):
    """Class for applying MixUp augmentation to the dataset."""
    ...
    def _mix_transform(self, labels):
        ...
        labels2 = labels["mix_labels"][0]
        labels["img"] = (labels["img"] * r + labels2["img"] * (1 - r)).astype(np.uint8)
        labels["instances"] = InstancesParsing.concatenate([labels["instances"], labels2["instances"]], axis=0)
        labels["cls"] = np.concatenate([labels["cls"], labels2["cls"]], 0)
        #pdb.set_trace()
        #see_rectangle(labels)
        return labels


class MixUpParsingWBC(BaseMixTransform):
    """Class for applying MixUp augmentation to the dataset."""
    ...
    def _mix_transform(self, labels):
        ...
        labels2 = labels["mix_labels"][0]
        labels["img"] = (labels["img"] * r + labels2["img"] * (1 - r)).astype(np.uint8)
        labels["instances"] = InstancesParsingWBC.concatenate([labels["instances"], labels2["instances"]], axis=0)
        labels["cls"] = np.concatenate([labels["cls"], labels2["cls"]], 0)
        #pdb.set_trace()
        #see_rectangle(labels)
        return labels
...
class MixUpParsingGroup(BaseMixTransform):
    """Class for applying MixUp augmentation to the dataset."""
    ...
    def _mix_transform(self, labels):
        ...
        labels["instances"] = InstancesParsing.concatenate([labels["instances"], labels2["instances"]], axis=0)
        labels["cls"] = np.concatenate([labels["cls"], labels2["cls"]], 0)
        labels["pathological_cls"] = np.concatenate([labels["pathological_cls"], labels2["pathological_cls"]], 0)
        if len(labels["group"]) == 0:
            max_value = 0  
        else:
            max_value = labels["group"].max()
        labels["group"] = np.concatenate([labels["group"], labels2["group"] + max_value], 0)
        #pdb.set_trace()
        #see_rectangle(labels)
        return labels


class MixUpParsingGroupMatch(BaseMixTransform):
    """Class for applying MixUp augmentation to the dataset."""
...
class RandomPerspective:
    ...
    def __call__(self, labels):
        ...
        #pdb.set_trace()
        #see_rectangle3(labels["img"].astype(np.uint8).copy(), labels["instances"].bboxes)
        return labels
...
class RandomPerspectiveParsing:
    """
    Implements random perspective and affine transformations on images and corresponding bounding boxes, segments, and
    keypoints. These transformations include rotation, translation, scaling, and shearing. The class also offers the
    option to apply these transformations conditionally with a specified probability.
    ...
    def apply_segments(self, segments, M):
        ...
        for segment in segments:
            n, num = segment.shape[:2]
            if n == 0:
                return [], segment
    
            xy = np.ones((n * num, 3), dtype=segment.dtype)
            segment = segment.reshape(-1, 2)
            xy[:, :2] = segment
            xy = xy @ M.T  # transform
            xy = xy[:, :2] / xy[:, 2:3]
            segment = xy.reshape(n, -1, 2)
            bboxes = np.stack([segment2box(xy, self.size[0], self.size[1]) for xy in segment], 0)
            segment[..., 0] = segment[..., 0].clip(bboxes[:, 0:1], bboxes[:, 2:3])
            segment[..., 1] = segment[..., 1].clip(bboxes[:, 1:2], bboxes[:, 3:4])
            segments_new.append(segment)
        segments_new = np.stack(segments_new, axis=0)
        #pdb.set_trace()
        return segments_new

    def apply_segments_instance(self, segments, M):
        """
        Apply affine to segments and generate new bboxes from segments.
    ...
    def __call__(self, labels):
        ...
        segments = instances.segments
        segments_instance = instances.segments_instance
        keypoints = instances.keypoints
        # Update bboxes if there are segments.
        #pdb.set_trace()
        if len(segments):
            segments = self.apply_segments(segments, M)
        if len(segments_instance):
            bboxes, segments_instance = self.apply_segments_instance(segments_instance, M)
        
        if keypoints is not None:
            keypoints = self.apply_keypoints(keypoints, M)
        new_instances = InstancesParsing(bboxes, segments, segments_instance, keypoints, bbox_format="xyxy", normalized=False)
        # Clip
        new_instances.clip(*self.size)
...
class RandomPerspectiveParsingWBC:
    """
    Implements random perspective and affine transformations on images and corresponding bounding boxes, segments, and
    keypoints. These transformations include rotation, translation, scaling, and shearing. The class also offers the
    option to apply these transformations conditionally with a specified probability.
    ...
    def affine_transform_segments(self, img, border, segments):
        """
        Applies a sequence of affine transformations centered around the image center.
        ...
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            if self.perspective:
                ...
                output_size = (w, h)  # keep same size
                transformed_mask = np.empty_like(segments, dtype=np.uint8)
            
                for i in range(n):
                    for j in range(c):
                        transformed_mask[i, j] = cv2.warpPerspective(segments[i, j], M, dsize=self.size, flags=cv2.INTER_NEAREST, borderValue=0)
            else:  # affine
                #pdb.set_trace()
                img = cv2.warpAffine(img, M[:2], dsize=self.size, borderValue=(114, 114, 114))
                n, c, h, w = segments.shape
                ...
                for i in range(n):
                    for j in range(c):
                        transformed_mask[i, j] = cv2.warpAffine(segments[i, j], M[:2], dsize=self.size, flags=cv2.INTER_NEAREST, borderValue=0)
        return img, M, s, transformed_mask
    ...
    def __call__(self, labels):
        ...
        segments = instances.segments
        if len(segments):
            img, M, scale, segments = self.affine_transform_segments(img, border, segments)
        else:
            img, M, scale = self.affine_transform(img, border)
        #pdb.set_trace()
        #cv2.imwrite('2.jpg', img)
        ...
        if keypoints is not None:
            keypoints = self.apply_keypoints(keypoints, M)
        new_instances = InstancesParsingWBC(bboxes, segments, segments_instance, keypoints, bbox_format="xyxy", normalized=False)
        # Clip
        new_instances.clip(*self.size)
...
class RandomPerspectiveParsingGroup(RandomPerspectiveParsing):
    def __init__(self, degrees=0.0, translate=0.1, scale=0.5, shear=0.0, perspective=0.0, border=(0, 0), pre_transform=None):
        super().__init__(degrees=degrees, translate=translate, scale=scale, shear=shear, perspective=perspective, border=border, pre_transform=pre_transform)
    ...
    def __call__(self, labels):
        ...
        cls = labels["cls"]
        group = labels["group"]
        pathological_cls = labels["pathological_cls"]
        instances = labels.pop("instances")
        # Make sure the coord formats are right
        instances.convert_bbox(format="xyxy")
        instances.denormalize(*img.shape[:2][::-1])
        ...
        labels["instances"] = new_instances[i]
        labels["cls"] = cls[i]
        labels["group"] = group[i]
        labels["pathological_cls"] = pathological_cls[i]
        labels["img"] = img
        labels["resized_shape"] = img.shape[:2]
...
class RandomPerspectiveParsingGroupMatch(RandomPerspectiveParsing):
    def __init__(self, degrees=0.0, translate=0.1, scale=0.5, shear=0.0, perspective=0.0, border=(0, 0), pre_transform=None):
        super().__init__(degrees=degrees, translate=translate, scale=scale, shear=shear, perspective=perspective, border=border, pre_transform=pre_transform)
...
class CopyPasteParsing:
    ...
    def __call__(self, labels):
        ...
        if self.p and len(instances.segments):
            ...
            for j in random.sample(list(indexes), k=round(self.p * n)):
                cls = np.concatenate((cls, cls[[j]]), axis=0)
                instances = InstancesParsing.concatenate((instances, ins_flip[[j]]), axis=0)
                cv2.drawContours(im_new, instances.segments_instance[[j]].astype(np.int32), -1, (1, 1, 1), cv2.FILLED)
...
class CopyPasteParsingWBC:
    ...
    def __call__(self, labels):
        ...
        if self.p and len(instances.segments):
            ...
            for j in random.sample(list(indexes), k=round(self.p * n)):
                cls = np.concatenate((cls, cls[[j]]), axis=0)
                instances = InstancesParsingWBC.concatenate((instances, ins_flip[[j]]), axis=0)
                cv2.drawContours(im_new, instances.segments_instance[[j]].astype(np.int32), -1, (1, 1, 1), cv2.FILLED)
...
class CopyPasteParsingGroup:
    ...
    def __call__(self, labels):
        ...
        if self.p and len(instances.segments):
            ...
            for j in random.sample(list(indexes), k=round(self.p * n)):
                cls = np.concatenate((cls, cls[[j]]), axis=0)
                pathological_cls = np.concatenate((pathological_cls, pathological_cls[[j]]), axis=0)
                group = np.concatenate((group, group[[j]]+max_group+1), axis=0)   
                instances = InstancesParsing.concatenate((instances, ins_flip[[j]]), axis=0)
                cv2.drawContours(im_new, instances.segments_instance[[j]].astype(np.int32), -1, (1, 1, 1), cv2.FILLED)
        ...
        labels["group"] = group
        labels["instances"] = instances
        labels["pathological_cls"] = pathological_cls
        return labels


class CopyPasteParsingGroupMatch:
    """
    Implements the Copy-Paste augmentation as described in the paper https://arxiv.org/abs/2012.07177. This class is
    responsible for applying the Copy-Paste augmentation on images and their corresponding instances.
    """
...
class Format:
    ...
    def __call__(self, labels):
        ...
        #pdb.set_trace()
        #cv2.imwrite('1.jpg', labels["img"].numpy().transpose((1, 2, 0)).astype(np.uint8).copy())
...
def see_segmentpart(labels):
    #pdb.set_trace()
    masks = labels["masks"]
    img = labels["img"].numpy().transpose((1, 2, 0)).astype(np.uint8).copy()
    h, w, _ = img.shape 
    classes = labels["cls"]
    for j in range(len(masks)):
        for k in range(len(masks[j])):
            if k == 0:
                color = colors(classes[j]+4)
                mh, mw = masks[j][4].shape
                if mh != h or mw != w:
                    mask = masks[j][4].numpy().astype(np.uint8)
                    mask = cv2.resize(mask, (w, h))
                    mask = mask.astype(bool)
                else:
                    mask = mask[j][k].numpy().astype(bool)
                img[mask] = color
            elif k == 4:
                color = colors(classes[j]+0)
                mh, mw = masks[j][0].shape
                if mh != h or mw != w:
                    mask = masks[j][0].numpy().astype(np.uint8)
                    mask = cv2.resize(mask, (w, h))
                    mask = mask.astype(bool)
                else:
                    mask = mask[j][k].numpy().astype(bool)
                ...
                color = colors(classes[j]+k)
                mh, mw = masks[j][k].shape
                if mh != h or mw != w:
                    mask = masks[j][k].numpy().astype(np.uint8)
                    mask = cv2.resize(mask, (w, h))
                    mask = mask.astype(bool)
                else:
                    mask = mask[j][k].numpy().astype(bool)
                img[mask] = color
    #pdb.set_trace()
    cv2.imwrite('image_with_segments.jpg', img)
    

def convert_yolo_to_cv2_bbox(yolo_bbox, img_width, img_height):
...
def see_rectangle(labels):
    pdb.set_trace()
    yolo_bboxes = labels["bboxes"].numpy()
    img = labels["img"].numpy().transpose((1, 2, 0)).astype(np.uint8).copy() 
    for i, yolo_bbox in enumerate(yolo_bboxes):
        cv2_bbox = convert_yolo_to_cv2_bbox(yolo_bbox, labels["resized_shape"][0], labels["resized_shape"][1])
        x_min, y_min, x_max, y_max = cv2_bbox
        x_coord, y_coord = (x_min + x_max)/2, (y_min + y_max)/2
        cv2.rectangle(img, (x_min, y_min), (x_max, y_max), (0, 255, 0), 2)
        group_id = labels["group"][i].numpy()[0]
        cv2.putText(img, f"{group_id}", (int(x_coord), int(y_coord)), 0, 1, (255, 255, 255), 1, cv2.LINE_AA,)
    pdb.set_trace()
    cv2.imwrite('image_with_bounding_boxes.jpg', img)
    
def see_rectangle2(img, yolo_bboxes):
    pdb.set_trace()
    for yolo_bbox in yolo_bboxes:
        cv2_bbox = convert_yolo_to_cv2_bbox(yolo_bbox, img.shape[1], img.shape[0])
        x_min, y_min, x_max, y_max = cv2_bbox
        cv2.rectangle(img, (x_min, y_min), (x_max, y_max), (0, 255, 0), 2)
    pdb.set_trace()
    cv2.imwrite('image_with_bounding_boxes.jpg', img)
    
def see_rectangle3(img, yolo_bboxes):
    pdb.set_trace()
...
class FormatParsing:
    ...
    def __call__(self, labels):
        ...
        if self.return_mask:
            if nl:
                ...
                masks = torch.zeros(
                    1 if self.mask_overlap else nl, num_part, img.shape[0] // self.mask_ratio, img.shape[1] // self.mask_ratio
                )
            labels["masks"] = masks
        ...
        if self.batch_idx:
            labels["batch_idx"] = torch.zeros(nl)
        #pdb.set_trace()
        #cv2.imwrite('2.jpg', labels["img"].numpy().transpose((1, 2, 0)).astype(np.uint8).copy())
        #see_segmentpart(labels)
        #see_rectangle(labels)
        return labels
    ...
    def _format_segments(self, instances, cls, w, h):
        ...
        segments = instances.segments
        if self.mask_overlap:
            masks, sorted_idx = polygons2masksparsing_overlap((h, w), segments, downsample_ratio=self.mask_ratio)
            masks = masks[None]  # (640, 640) -> (1, 640, 640)
            instances = instances[sorted_idx]
            cls = cls[sorted_idx]
        else:
            masks = polygons2masksparsing((h, w), segments, color=1, downsample_ratio=self.mask_ratio)

        return masks, instances, cls


class FormatParsingWBC:
    """
    Formats image annotations for object detection, instance segmentation, and pose estimation tasks. The class
    standardizes the image and instance annotations to be used by the `collate_fn` in PyTorch DataLoader.
    ...
    def __call__(self, labels):
        ...
        assert isinstance(instances, InstancesParsingWBC)
        #pdb.set_trace()
        if self.return_mask:
            if nl:
                masks = instances.segments
                masks = torch.from_numpy(masks.copy())
                masks = F.interpolate(masks.float(), scale_factor=1/self.mask_ratio, mode='nearest').to(dtype=torch.uint8)
                #masks, instances, cls = self._format_segments(instances, cls, w, h)
            else:
                masks = torch.zeros(
                    1 if self.mask_overlap else nl, num_part, img.shape[0] // self.mask_ratio, img.shape[1] // self.mask_ratio
...
class FormatParsingGroup:
    ...
    def __call__(self, labels):
        ...
        h, w = img.shape[:2]
        cls = labels.pop("cls")
        group = labels.pop("group")
        num_part = labels.pop("num_part")
        instances = labels.pop("instances")
        instances.convert_bbox(format=self.bbox_format)
        instances.denormalize(w, h)
        pathological_cls = labels.pop("pathological_cls")
        nl = len(instances)
        
        #pdb.set_trace()
        if self.return_mask:
            if nl:
                masks, instances, cls, group, pathological_cls = self._format_segments(instances, cls, group, w, h, pathological_cls)
                masks = torch.from_numpy(masks)
            else:
                masks = torch.zeros(
                    1 if self.mask_overlap else nl, num_part, img.shape[0] // self.mask_ratio, img.shape[1] // self.mask_ratio
        ...
        labels["cls"] = torch.from_numpy(cls) if nl else torch.zeros(nl)
        labels["pathological_cls"] = torch.from_numpy(pathological_cls) if nl else torch.zeros(nl)
        labels["group"] = torch.from_numpy(group) if nl else torch.zeros(nl)
        labels["bboxes"] = torch.from_numpy(instances.bboxes) if nl else torch.zeros((nl, 4))
        if self.return_keypoint:
            labels["keypoints"] = torch.from_numpy(instances.keypoints)
            if self.normalize:
    ...
    def _format_segments(self, instances, cls, group, w, h, pathological_cls):
        #pdb.set_trace()
        """Convert polygon points to bitmap."""
        segments = instances.segments
        if self.mask_overlap:
            ...
            pathological_cls = pathological_cls[sorted_idx]
        else:
            masks = polygons2masksparsing((h, w), segments, color=1, downsample_ratio=self.mask_ratio)

        return masks, instances, cls, group, pathological_cls

class FormatParsingGroupMatch:
    """
    Formats image annotations for object detection, instance segmentation, and pose estimation tasks. The class
    standardizes the image and instance annotations to be used by the `collate_fn` in PyTorch DataLoader.
    ...
    def __call__(self, labels):
        ...
        if self.return_mask:
            if nl:
                masks, instances, cls, group = self._format_segments(instances, cls, group, w, h)
                masks = torch.from_numpy(masks)
            else:
                masks = torch.zeros(
                    1 if self.mask_overlap else nl, num_part, img.shape[0] // self.mask_ratio, img.shape[1] // self.mask_ratio
    ...
    def _format_segments(self, instances, cls, group, w, h):
        #pdb.set_trace()
        """Convert polygon points to bitmap."""
        segments = instances.segments
        if self.mask_overlap:
        ...
        return masks, instances, cls, group
...
def v8_transforms_parsing(dataset, imgsz, hyp, stretch=False):
    """Convert images to a size suitable for YOLOv8 training."""
    #pdb.set_trace()
    pre_transform = Compose(
        [
            MosaicParsing(dataset, imgsz=imgsz, p=hyp.mosaic),
            CopyPasteParsing(p=hyp.copy_paste),
            RandomPerspectiveParsing(
                degrees=hyp.degrees,
                translate=hyp.translate,
                scale=hyp.scale,
                shear=hyp.shear,
    ...
    return Compose(
        [
            pre_transform,
            MixUpParsing(dataset, pre_transform=pre_transform, p=hyp.mixup),
            Albumentations(p=1.0),
            RandomHSV(hgain=hyp.hsv_h, sgain=hyp.hsv_s, vgain=hyp.hsv_v),
            RandomFlip(direction="vertical", p=hyp.flipud),
            RandomFlip(direction="horizontal", p=hyp.fliplr, flip_idx=flip_idx),
...
def v8_transforms_parsing_WBC(dataset, imgsz, hyp, stretch=False):
    """Convert images to a size suitable for YOLOv8 training."""
    #pdb.set_trace()
    pre_transform = Compose(
        [
            MosaicParsingWBC(dataset, imgsz=imgsz, p=hyp.mosaic),
            CopyPasteParsingWBC(p=hyp.copy_paste),
            RandomPerspectiveParsingWBC(
                degrees=hyp.degrees,
                translate=hyp.translate,
                scale=hyp.scale,
                shear=hyp.shear,
    ...
    return Compose(
        [
            pre_transform,
            MixUpParsingWBC(dataset, pre_transform=pre_transform, p=hyp.mixup),
            Albumentations(p=1.0),
            RandomHSV(hgain=hyp.hsv_h, sgain=hyp.hsv_s, vgain=hyp.hsv_v),
            RandomFlip(direction="vertical", p=hyp.flipud),
            RandomFlip(direction="horizontal", p=hyp.fliplr, flip_idx=flip_idx),
...
def v8_transforms_parsing_group(dataset, imgsz, hyp, stretch=False):
    """Convert images to a size suitable for YOLOv8 training."""
    #pdb.set_trace()
    pre_transform = Compose(
        [
            MosaicParsingGroup(dataset, imgsz=imgsz, p=hyp.mosaic),
            CopyPasteParsingGroup(p=hyp.copy_paste),
            RandomPerspectiveParsingGroup(
                degrees=hyp.degrees,
                translate=hyp.translate,
                scale=hyp.scale,
                shear=hyp.shear,
    ...
    return Compose(
        [
            pre_transform,
            MixUpParsingGroup(dataset, pre_transform=pre_transform, p=hyp.mixup),
            Albumentations(p=1.0),
            RandomHSV(hgain=hyp.hsv_h, sgain=hyp.hsv_s, vgain=hyp.hsv_v),
            RandomFlip(direction="vertical", p=hyp.flipud),
            RandomFlip(direction="horizontal", p=hyp.fliplr, flip_idx=flip_idx),
...
def v8_transforms_parsing_group_match(dataset, imgsz, hyp, stretch=False):
    """Convert images to a size suitable for YOLOv8 training."""
    #pdb.set_trace()
    pre_transform = Compose(
        [
            MosaicParsingGroupMatch(dataset, imgsz=imgsz, p=hyp.mosaic),
            CopyPasteParsingGroupMatch(p=hyp.copy_paste),
            RandomPerspectiveParsingGroupMatch(
                degrees=hyp.degrees,
                translate=hyp.translate,
                scale=hyp.scale,
                shear=hyp.shear,
    ...
    return Compose(
        [
            pre_transform,
            MixUpParsingGroupMatch(dataset, pre_transform=pre_transform, p=hyp.mixup),
            Albumentations(p=1.0),
            RandomHSV(hgain=hyp.hsv_h, sgain=hyp.hsv_s, vgain=hyp.hsv_v),
            RandomFlip(direction="vertical", p=hyp.flipud),
            RandomFlip(direction="horizontal", p=hyp.fliplr, flip_idx=flip_idx),
