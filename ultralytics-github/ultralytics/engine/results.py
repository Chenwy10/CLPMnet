...
from ultralytics.utils.plotting import Annotator, colors, save_one_box, AnnotatorParsing
...
class ResultsParsing(Results):
    ...
    def plot(
        ...
        if pred_masks and show_masks:
            ...
            for x in idx:
                color_new = []
                for i in range(pred_masks.data.shape[1]):
                    color_new.append(colors(x*pred_masks.data.shape[1]+i, True))
                colors_new.append(color_new)
            #pdb.set_trace()
            annotator.masks(pred_masks.data, colors=np.array(colors_new), im_gpu=im_gpu)
...
class ResultsParsingMatch(Results):
    def __init__(self, orig_img, path, names, boxes=None, masks=None, match_pair=None, pathological_cls=None, pathological_names=None,probs=None, keypoints=None, obb=None, speed=None) -> None:
        super().__init__(orig_img, path, names, boxes, masks, probs, keypoints, obb, speed)
        self.match_pair = match_pair
        self.pathological_cls = pathological_cls
        self.pathological_names = pathological_names
    ...
    def plot(
        ...
        if pred_boxes is not None and show_boxes:
            #pdb.set_trace()
            pred_pathological_cls = reversed(self.pathological_cls) 
            for i, d in enumerate(reversed(pred_boxes)):
                c, conf, id = int(d.cls), float(d.conf) if conf else None, None if d.id is None else int(d.id.item())
                name = ("" if id is None else f"id:{id} ") + names[c]
                label = (f"{name} {conf:.2f}" if conf else name) if labels else None
                pathological_name = self.pathological_names[int(pred_pathological_cls[i])]
                label = (f"{label} {pathological_name}")
                box = d.xyxyxyxy.reshape(-1, 4, 2).squeeze() if is_obb else d.xyxy.squeeze()
                annotator.box_label(box, label, color=colors(c, True), rotated=is_obb)
        ...
        if self.match_pair is not None:
            annotator.matchs(self.match_pair, pred_boxes, conf_thres=0.5)
        
        #if self.pathological_cls is not None:
        #    annotator.pathological_texts()    
