...
def compute_hist(image_list, image_root, all_segms, num_parsing):
    n_class = num_parsing
    hist = np.zeros((n_class, n_class))
    
    #pdb.set_trace()
    i = 0
    for image_path in tqdm(image_list, desc='Calculating IoU ..'):
        image_filename = os.path.basename(image_path)
        base_name, _ = os.path.splitext(image_filename)
        image_filename = base_name + '.png'
        image_png = os.path.join(image_root, image_filename)
        
        #pdb.set_trace()
        label = cv2.imread(image_png, 0)
        tmp = all_segms[i]
...
def parsing_iou(all_segms, image_list, image_root, num_parsing):
    #pdb.set_trace()
    #predict_list = glob.glob(predict_root + '/*.png')
    print('The predict size: {}'.format(len(image_list)))

    hist = compute_hist(image_list, image_root, all_segms, num_parsing)
...
def cal_one_mean_iou(image_array, label_array, num_parsing):
    hist = fast_hist(label_array, image_array, num_parsing).astype(np.float64)
...
def evaluate_parsing(all_results, all_segms, all_scores, eval_ap, score_thresh, num_parsing, im_dir, ann_fn, image_list, image_root):
    ...
    #pdb.set_trace()
    _iou, _miou, mean_acc, pixel_acc = parsing_iou(all_segms, image_list, image_root, num_parsing)
    ...
    if eval_ap:
        all_ap_p, all_pcp = eval_parsing_ap(all_results, all_scores, score_thresh, im_dir, ann_fn, num_parsing)
