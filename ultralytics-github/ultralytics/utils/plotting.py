...
class AnnotatorParsing(Annotator):
    def __init__(self, im, line_width=None, font_size=None, font="Arial.ttf", pil=False, example="abc"):
    ...
    def matchs(self, match_pairs, box, radius=5, kpt_line=True, conf_thres=0.5):
        #pdb.set_trace()
        if self.pil:
            # Convert to numpy first
            self.im = np.asarray(self.im).copy()
        ...
        for i in range(len(box)):
            x1, y1, x2, y2 = box[i].xyxy[0]
            x_coord, y_coord = (x1 + x2)/2, (y1 + y2)/2
            x_coord, y_coord = x_coord.cpu().numpy(), y_coord.cpu().numpy()
            cv2.putText(
                self.im,
                f"Index: {i}",
        ...
        for match_pair in match_pairs:
            head_index, tail_index, logits = match_pair
            x1, y1, x2, y2 = box[head_index.int()].xyxy[0]
            x_coord_head, y_coord_head = (x1 + x2)/2, (y1 + y2)/2 
            x_coord_head, y_coord_head = x_coord_head.cpu().numpy(), y_coord_head.cpu().numpy()
            
            x1, y1, x2, y2 = box[tail_index.int()].xyxy[0]
            x_coord_tail, y_coord_tail = (x1 + x2)/2, (y1 + y2)/2
            x_coord_tail, y_coord_tail = x_coord_tail.cpu().numpy(), y_coord_tail.cpu().numpy()
            if logits < conf_thres:
                continue
            cv2.circle(self.im, (int(x_coord_head), int(y_coord_head)), radius, [int(x) for x in self.kpt_color[0]], -1, lineType=cv2.LINE_AA)
            cv2.circle(self.im, (int(x_coord_tail), int(y_coord_tail)), radius, [int(x) for x in self.kpt_color[1]], -1, lineType=cv2.LINE_AA)
            
            if kpt_line:
                cv2.line(self.im, (int(x_coord_head), int(y_coord_head)), (int(x_coord_tail), int(y_coord_tail)), [int(x) for x in self.limb_color[0]], thickness=2, lineType=cv2.LINE_AA)
            ...
            if len(masks):
                ...
                if masks.ndim == 3:
                    ...
                    if idx.shape[0] == masks.shape[0]:  # overlap_masks=False
                        ...
                        image_masks = masks[[i]]  # (1, 640, 640)
                        nl = idx.sum()
                        index = np.arange(nl).reshape((nl, 1, 1, 1)) + 1
                        image_masks = np.repeat(image_masks, nl, axis=0)
                        image_masks = np.where(image_masks == index, 1.0, 0.0)
    
                    im = np.asarray(annotator.im).copy()
                    for j in range(len(image_masks)):
                        for k in range(len(image_masks[j])):
                            if labels or conf[j] > conf_thres:
                                color = colors(classes[j]+k)
                                mh, mw = image_masks[j][k].shape
                                if mh != h or mw != w:
                                    mask = image_masks[j][k].astype(np.uint8)
                                    mask = cv2.resize(mask, (w, h))
                                    mask = mask.astype(bool)
                                else:
                                    mask = image_masks[j][k].astype(bool)
                                with contextlib.suppress(Exception):
                                    im[y : y + h, x : x + w, :][mask] = (
                                        im[y : y + h, x : x + w, :][mask] * 0.4 + np.array(color) * 0.6
                                    )
